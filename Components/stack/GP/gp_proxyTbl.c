...
static uint8 pt_updateAlias( uint8* pNew );
static uint8 pt_updateSecurity( uint8* pNew );
static uint8 pt_updateSecFrameCounter( uint8* pNew );
...
uint8 pt_ZclReadGetProxyEntry( uint16 nvId, uint8* pData, uint8* len )
{
  ...
  uint8  gpdEntry[PROXY_TBL_ENTRY_LEN] = {0};
  uint8  gpdEntryLen = 0;
  uint8  status;
  uint16 options;
...
static uint16 gp_pairingSetProxyTblOptions ( uint32 pairingOpt )
{
  ...
  {
    PROXY_TBL_SET_CGROUP( &proxyOpt[0] );
  }
  else
  {
    PROXY_TBL_CLR_CGROUP( &proxyOpt[0] );
  }
...
{
  uint8  entryVector[PROXY_TBL_ENTRY_LEN] = {0};
  uint8  entryLen = 0;
  uint8  addSink;
  uint16 entryOptions = 0;
  uint8  securityOpt = 0;
  uint8  searchCounter = 0x0A;
  uint16 grpAlias = 0xFFFF;
  ...
  // to add the invalid entries
  gp_PopulateProxyTblEntry( ( uint8* )&grpAlias, sizeof ( uint16 ), entryVector, &entryLen );
...
{
  uint8 i;
  uint8 newEntry[PROXY_TBL_ENTRY_LEN] = {0};
  uint8 currEntry[PROXY_TBL_ENTRY_LEN] = {0};
  uint16 proxyTableIndex;
  uint8 status;
  ...
  if ( pt_updateSecFrameCounter( newEntry ) )
  {
    PROXY_TBL_SET_SEC_CAP( &currEntry[PROXY_TBL_ENTRY_SEC_FRAME] );
  }
...
static uint8 pt_updateAlias( uint8* pNew )
{
  ...
  newAlias = PROXY_TBL_GET_ASSIGNED_ALIAS( newEntry[PROXY_TBL_ENTRY_OPT + 1] );
  
  return newAlias;
}

/*********************************************************************
 * @fn          pt_updateSecurity
 *
...
static uint8 pt_updateSecurity( uint8* pNew )
{
  ...
  secUse = PROXY_TBL_GET_SEC_USE( newEntry[PROXY_TBL_ENTRY_OPT + 1] );

  return secUse;
}

/*********************************************************************
 * @fn          pt_updateSecFrameCounter
 *
...
static uint8 pt_updateSecFrameCounter( uint8* pNew )
{
  ...
  secCap = PROXY_TBL_GET_SEC_CAP( newEntry[PROXY_TBL_ENTRY_OPT] );
...
static uint8 pt_addSinkGroup( uint8* pNew, uint8* pCurr )
{
  ...
  uint8 newEntry[PROXY_TBL_ENTRY_LEN] = {0};
  uint8 currEntry[PROXY_TBL_ENTRY_LEN] = {0};
  uint8 freeSinkEntry[LSINK_ADDR_LEN] = {0xFF};
  ...
  if ( newSinkGroup )
  {
    if ( osal_memcmp( freeSinkEntry, &currEntry[PROXY_TBL_ENTRY_2ND_GRP_ADDR], sizeof ( uint16 ) ) )
    {
      if ( osal_memcmp( freeSinkEntry, &currEntry[PROXY_TBL_ENTRY_1ST_GRP_ADDR], sizeof ( uint16 ) ) )
      {
        osal_memcpy( &currEntry[PROXY_TBL_ENTRY_1ST_GRP_ADDR], &newEntry[PROXY_TBL_ENTRY_1ST_GRP_ADDR], sizeof ( uint16 ) );
        osal_memcpy( pCurr, &currEntry, PROXY_TBL_ENTRY_LEN );
        return TRUE;
      }
      else
      {
        osal_memcpy( &currEntry[PROXY_TBL_ENTRY_2ND_GRP_ADDR], &newEntry[PROXY_TBL_ENTRY_2ND_GRP_ADDR], sizeof ( uint16 ) );
        osal_memcpy( pCurr, &currEntry, PROXY_TBL_ENTRY_LEN );
        return TRUE;
      }
    }
