...
uint8  zclGp_gppFunctionality[3] = {0};
uint8  zclGp_gppActiveFunctionality[3] = {0};

uint8  zclGp_gpSharedSecurityKeyType = GP_SHARED_SEC_KEY_TYPE;
uint8  zclGp_gpSharedSecurityKey[SEC_KEY_LEN] = {0};
...
static const cId_t greenPower_EP_InClusterList[GREEN_POWER_EP_MAX_INCLUSTERS] =
{
  ZCL_CLUSTER_ID_GREEN_POWER
};
...
static SimpleDescriptionFormat_t greenPower_EP_SimpleDesc =
{
  ...
  0,                                     //  int   AppDevVer:4;
  0,                                     //  int   AppFlags:4;
  GREEN_POWER_EP_MAX_INCLUSTERS,         //  byte  AppNumInClusters;
  (cId_t *)greenPower_EP_InClusterList,  //  byte *pAppInClusterList;
...
static void zclGp_GpProxyTableReqCB( zclGpProxyTableRequest_t *pCmd )
{
  ...
  if ( GP_IS_REQ_TPY_INDEX ( proxyTblReqPayload.options ) )
  {
    ...
    {    
      ...
      status = pt_ZclReadGetProxyEntry ( ( ZCD_NV_PROXY_TABLE_START + i ), NULL, &entryLen );
    ...
    if ( buf != NULL )
    {
      ...
      for ( i = proxyTblReqPayload.index; i <= GPP_MAX_PROXY_TABLE_ENTRIES ; i++ )
      {  
        status = pt_ZclReadGetProxyEntry ( ( ZCD_NV_PROXY_TABLE_START + i ), pBuf, &entryLen );
        if ( status == NV_OPER_FAILED )
        {
          break;
        }
 ...
 *
 * @brief   Callback from the ZCL GreenPower Cluster Library when
 *          it received a Gp Response command.
 *
 * @param   pCmd - command payload
 *
 * @return  none
...
static void zclGp_GpResponseCommandCB(zclGpResponse_t *pCmd)
{
  ...
  if(pCmd->tempMasterShortAddr == _NIB.nwkDevAddress)
  {
    ...
    if(pCmd->dstAddr == _NIB.nwkDevAddress)
    {
    ...
    for ( i = 0; i <= GPP_MAX_PROXY_TABLE_ENTRIES ; i++ )
    {
      ...
      if( GP_IS_APPLICATION_ID_GPD( payload->options ) )
      {
        if ( osal_memcmp( &currEntry[PROXY_TBL_ENTRY_GPD_ID + 4], &payload->gpdId, sizeof ( uint32 ) ) )
        {
          ...
          osal_memset( currEntry, 0xFF, PROXY_TBL_ENTRY_LEN );
        }
      }
      else if( GP_IS_APPLICATION_ID_IEEE( payload->options ) )
      {   
...
uint8 gp_ProxyTblInit( uint8 resetTable )
{
  ...
  osal_memset( emptyEntry, 0xFF, PROXY_TBL_ENTRY_LEN );
...
//Active/Inactive entries in the proxy table are not supported    
      ...
      else if(pInd->GPDFSecLvl != GP_GET_SEC_LEVEL(currEntry[PROXY_TBL_ENTRY_SEC_OPT]))
      {
        securityCheckFail = TRUE;
      }
      //Mapping of security key type (section A.1.4.1.3 Table 12)
      else if((GP_GET_SEC_KEY_TYPE(currEntry[PROXY_TBL_ENTRY_SEC_OPT]) <= 0x03)
              && (pInd->GPDFKeyType == 1))
      {
        securityCheckFail = TRUE;
      }
      else if(  ((GP_GET_SEC_KEY_TYPE(currEntry[PROXY_TBL_ENTRY_SEC_OPT]) == 0x07) ||
                 (GP_GET_SEC_KEY_TYPE(currEntry[PROXY_TBL_ENTRY_SEC_OPT]) == 0x04)) 
               && (pInd->GPDFKeyType == 0) )
      {
        securityCheckFail = TRUE;
        
      }
      else if( (GP_GET_SEC_KEY_TYPE(currEntry[PROXY_TBL_ENTRY_SEC_OPT]) == 0x05) ||
               (GP_GET_SEC_KEY_TYPE(currEntry[PROXY_TBL_ENTRY_SEC_OPT]) == 0x06) )
      {
        //keytype reserved
        securityCheckFail = TRUE;
      }
...
static void GP_ProccessNotification(gp_DataInd_t *gp_DataInd)
{
  ...
  zclGp_SendGpNotificationCommand ( &gpNotification );
}

/*********************************************************************
 * @fn          GP_SecReq
 *
 * @brief       Primitive from dGP stub to GP EndPoint notify about a protected GPDF
 *
 * @param       gp_SecReq 
 *
 * @return      none
...
uint8 GP_SecReq(gp_SecReq_t *gp_SecReq)
{
  ...
  if(NULL == gp_DataIndFindDuplicate(gp_SecReq->dGPStubHandle, gp_SecReq->gp_SecData.GPDFSecLvl))
  {
    if(gp_SecReq->gp_SecData.GPDFSecLvl)
    {
      gp_DataInd_t* temp;
      uint8   KeyType;
