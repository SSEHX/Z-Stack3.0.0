...
#define TCLK_DEV_ENTRY_MAX_SEED_SHIFTS     15
/******************************************************************************
 * TYPEDEFS
 */
typedef struct
...
APSME_TCLinkKeyFrmCntr_t TCLinkKeyFrmCntr[ZDSECMGR_TC_DEVICE_MAX];
 ...
 CONST uint8 gZDSECMGR_TC_DEVICE_MAX = ZDSECMGR_TC_DEVICE_MAX;
 CONST uint8 gZDSECMGR_TC_DEVICE_IC_MAX = ZDSECMGR_TC_DEVICE_IC_MAX;
/******************************************************************************
 * PRIVATE FUNCTIONS
 *
 *   ZDSecMgrAddrStore
...
void APSME_TCLinkKeyInit(uint8 setDefault)
{
  ...
  //osal_nv_item_init(ZCD_NV_TCLK_DEFAULT, SEC_KEY_LEN,(void*) &defaultTCLinkKey);
...
{
  uint8 selectedId = gZDSECMGR_TC_DEVICE_MAX;
  ...
  if(entryFound)
  {
    ...
    switch(TCLKDevEntry.keyAttributes)
    {
      ...
      case ZG_VERIFIED_KEY:
        //Only verify the frame counter for Verified keys
...
ZStatus_t APSME_TCLinkKeyLoad( uint16 dstAddr, SSP_Info_t* si )
{
  ...
  if(extAddrFound)
  {
    ...
    {
      uint8   i = entryIndex - ZCD_NV_TCLK_TABLE_START;
