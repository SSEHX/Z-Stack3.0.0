...
void ZDO_ProcessNodeDescRsp ( zdoIncomingMsg_t *inMsg )
{
  ...
  if(requestNewTrustCenterLinkKey)
  {
    if(!APSME_IsDistributedSecurity())
    {
      ...
      if(inMsg->srcAddr.addr.shortAddr == destAddr.addr.shortAddr)
      {
...
void ZDO_ProcessSimpleDescRsp( zdoIncomingMsg_t *inMsg )
{
  ...
  {
    len += 1 + 2 + 1;  // [Status] + [dstAddr] + [len]
    len += inMsg->asdu[3];  //Simple Descriptor lenght
    bdb_SendMsg(bdb_TaskID,bdbCommissioningProcedureState.bdbCommissioningState,*(uint8*)inMsg->asdu,len, inMsg->asdu);
  }
#endif
}
...
void ZDO_ProcessMgmtPermitJoinReq( zdoIncomingMsg_t *inMsg )
{
  ...
  {
    //If zgAllowRemoteTCPolicyChange is set to FALSE, the request cannot affect the 
    //Trust Center policies
    if(zgAllowRemoteTCPolicyChange == 0)
    {
      return;
    }
  }
