 ...
 * @return  none
 */
void bdb_ZclIdentifyCmdInd( zclIdentify_t *pCmd, uint8 endpoint )
{
  ...
  {
    if ( pCmd->identifyTime > 0 )
    {
      *((uint16*)identifyAttrRec.attr.dataPtr) += pCmd->identifyTime;
    }
    else if ( pCmd->identifyTime <= 0 )
    {
    ...
    {
      pfnIdentifyTimeChangeCB(pCmd->srcAddr->endPoint);
    }
  }
}
#endif
...
ZStatus_t bdb_setActiveCentralizedLinkKey(bool useGlobal, uint8* pBuf)
{
  ...
  if(useGlobal)
  {
    APSME_AddTCLinkKey((uint8*)defaultTCLinkKey,extAddr);
    Status = ZSuccess;
  } 
  else
  {
      ...
      if(ZG_DEVICE_JOINING_TYPE)
      {
        ...
        if(!osal_isbufset( AIB_apsTrustCenterAddress, 0x00, Z_EXTADDR_LEN ))
        {
          ...
          if(!APSME_IsDistributedSecurity())
          {
            ...
            //And the TCLK procedure got not complete, then reset the device parameters to FN
            if((keyAttributes != ZG_NON_R21_NWK_JOINED) && (keyAttributes != ZG_VERIFIED_KEY))
            {
...
void bdb_NetworkRestoredResumeState(void)
{
  ...
  if(ZG_DEVICE_ENDDEVICE_TYPE)
  {
    ...
    if(bdbCommissioningProcedureState.bdbCommissioningState == BDB_COMMISSIONING_STATE_FINDING_BINDING)
    {
      ...
      {
        if(pRespondentTemp->attempts == BDB_SIMPLE_DESC_PENDING_PARENT_LOST)
        {
          pRespondentTemp->attempts = 0;
    ...
    if(restoreSimpleDesc)
    {
      ...
      osal_start_timerEx(bdb_TaskID,BDB_RESPONDENT_SIMPLEDESC_TIMEOUT, 1000);
    }
  }
#endif
}
     ...
     || ((bdbCommissioningProcedureState.bdbCommissioningState == BDB_PARENT_LOST) && (bdbCommissioningProcedureState.bdb_ParentLostSavedState == BDB_COMMISSIONING_STATE_FINDING_BINDING)))
  {
    switch(bdbCommissioningState)
    {
      ...
      case BDB_COMMISSIONING_STATE_FINDING_BINDING:
        ...
        if(((FINDING_AND_BINDING_PERIODIC_ENABLE == FALSE) || (bdb_FB_InitiatorCurrentCyclesNumber == 0)) && (bdb_getRespondentRetry(pRespondentHead) == NULL) && (osal_get_timeoutEx( bdb_TaskID, BDB_RESPONDENT_SIMPLEDESC_TIMEOUT) == 0))
        {
...
UINT16 bdb_event_loop(byte task_id, UINT16 events)
{
  ...
  if ( events & SYS_EVENT_MSG )
  {
    ...
    while ( (msg_ptr = osal_msg_receive( bdb_TaskID )) )
    {
      ...
      if(((bdbInMsg_t*)msg_ptr)->hdr.event == bdbCommissioningProcedureState.bdbCommissioningState)
      {
    ...
    if ( FINDING_AND_BINDING_PERIODIC_ENABLE == TRUE )
    {
      ...
      if(bdb_CurrEpDescriptor != NULL) //just a safty check. The fact that we got to this functuon at all means that this cannot be NULL
      {
        ...
        if(bdb_FB_InitiatorCurrentCyclesNumber > 0)
        {
          ...
          if ((osal_get_timeoutEx(bdb_TaskID, BDB_RESPONDENT_SIMPLEDESC_TIMEOUT) == 0) && (bdb_getRespondentRetry(pRespondentHead) == NULL))
          {
            //Send identify query with the endpoint requested
  ...
  if(events & BDB_RESPONDENT_SIMPLEDESC_TIMEOUT)
  {
    // Send Simple Descriptor request to a respondent node
    bdb_SendSimpleDescReq();

    return (events ^ BDB_RESPONDENT_SIMPLEDESC_TIMEOUT);
  }
#endif
