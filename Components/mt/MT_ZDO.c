...
static void MT_ZdoStartupFromAppEx(uint8 *pBuf);
...
uint8 MT_ZdoCommandProcessing(uint8* pBuf)
{
  ...
  switch (pBuf[MT_RPC_POS_CMD1])
  {
    ...
    case MT_ZDO_STARTUP_FROM_APP:
      ...
      break;
    case MT_ZDO_STARTUP_FROM_APP_EX:
      MT_ZdoStartupFromAppEx(pBuf);
      break;
#endif
...
/***************************************************************************************************
 * @fn      MT_ZdoStartupFromAppEx
 *
 * @brief   Handle a Startup from App request Extended.
 *
 * @param   pBuf  - MT message data
 *
 * @return  void
...
static void MT_ZdoStartupFromAppEx(uint8 *pBuf)
{
  ...
  retValue = ZDOInitDeviceEx(delay, mode);
